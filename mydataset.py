...
class MaskBaseDataset(data.Dataset):
    ...
    class AgeGroup:
        map_label = lambda x: 0 if int(x) < 30 else 1 if int(x) < 58 else 2  #58이상은 60으로 학습
    ...
    # mean=(0.548, 0.504, 0.479), std=(0.237, 0.247, 0.246) 에서 변경
    ...
    def calc_statistics(self):
        ...
        if not has_statistics:
            ...
            for image_path in self.image_paths[:3000]:
                image = np.array(Image.open(image_path)).astype(np.int16)       #원래 int32
...
class MaskSplitByProfileDataset(MaskBaseDataset):       #데이터 유출 해결
    """
...
class TestDataset(data.Dataset):
    # mean=(0.548, 0.504, 0.479), std=(0.237, 0.247, 0.246) 변경
